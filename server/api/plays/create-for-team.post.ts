...
export default defineEventHandler(async (event) => {
  ...
  if (playbookId) {
    const { data: sharedPlaybook, error: sharedPlaybookErr } = await admin
      .from('team_playbooks')
      .select('playbook_id')
      .eq('team_id', team_id)
    ...
    if (sharedPlaybookErr) {
      throw createError({ statusCode: 500, statusMessage: sharedPlaybookErr.message ?? 'Failed to verify playbook access' })
    }

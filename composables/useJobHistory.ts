...
export function useJobHistory() {
  ...
  async function deleteJob(jobId: string): Promise<boolean> {
    ...
    try {
      const { data: ownedJob, error: fetchErr } = await client
        .from('sim_jobs')
        .select('id')
        .eq('id', jobId)
        .eq('user_id', user.value.id)
      ...
      await client.from('sim_insights').delete().eq('job_id', jobId).eq('user_id', user.value.id)
      // Remove notifications linked to this job
      await client.from('notifications').delete().eq('user_id', user.value.id).eq('metadata->>job_id', jobId)
  ...
  async function deleteAllJobs(): Promise<boolean> {
    ...
    try {
      ...
      await client.from('sim_insights').delete().eq('user_id', user.value.id).in('job_id', jobIds)
      // Remove all notifications linked to these jobs
      for (const jid of jobIds) {
        await client.from('notifications').delete().eq('user_id', user.value.id).eq('metadata->>job_id', jid)
      }
      const { error: err } = await client
        .from('sim_jobs')
        .delete()

    ...
    <div class="p-3 space-y-3 overflow-y-auto custom-scrollbar flex-1">
      ...
      <div class="space-y-3">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1">
            <div class="flex items-center justify-between">
              ...
              <span v-if="selectedPlayer.number" class="text-[12px] font-mono text-muted-foreground">#{{ selectedPlayer.number }}</span>
            </div>
            ...
            <!-- Combined Role Selector -->
              ...
              >
                <SelectTrigger class="h-6 text-[12px] bg-muted/30 border-border/50 col-span-2">
                ...
                <SelectContent>
                  ...
                  >
                    <div class="flex items-center justify-between w-full min-w-[120px]">
                      ...
                      <span class="text-muted-foreground ml-2 text-[11px] uppercase">{{ role.position }}</span>
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        <div v-if="playType === 'defense' && !isRusher(selectedPlayer)" class="space-y-3 pt-2 border-t border-border/50">
          <p class="text-[12px] font-bold text-muted-foreground uppercase tracking-wider">Defensive Coverage</p>

          <div class="space-y-1.5">
            <div class="flex items-center justify-between">
              <label class="text-[12px] text-muted-foreground">Coverage Radius</label>
              <span class="text-[12px] font-mono">{{ selectedPlayer.coverageRadius ?? 5 }} yd</span>
            </div>
            <input
              type="range"
              min="1"
          ...
          <div class="space-y-1.5">
            <label class="text-[12px] text-muted-foreground">Zone position</label>
            <button
              type="button"
              class="w-full px-2 py-1.5 text-[12px] font-medium rounded border transition-colors"
          ...
          <div class="space-y-1.5">
            <label class="text-[12px] text-muted-foreground">Alignment</label>
            <div class="grid grid-cols-4 gap-1">
               <button
                v-for="align in ['tight', 'normal', 'soft', 'off']"
                :key="align"
                class="px-1 py-1 text-[11px] rounded border transition-colors capitalize"
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="bg-muted/30 rounded px-2.5 py-1.5 border border-border/50">
          <p class="text-[11px] text-muted-foreground font-medium mb-0.5">YARD LINE</p>
          <p class="text-xs font-mono font-bold text-foreground">{{ yardLine }}</p>
        </div>
        <div class="bg-muted/30 rounded px-2.5 py-1.5 border border-border/50">
          <p class="text-[11px] text-muted-foreground font-medium mb-0.5">FROM SIDELINE</p>
      ...
      <div v-if="playType === 'offense'" class="space-y-2 pt-2 border-t border-border/50">
        <div class="flex items-center justify-between">
          <p class="text-[12px] font-bold text-muted-foreground uppercase tracking-wider">Route Segments</p>
          <div class="flex items-center gap-1">
            <Button
              v-if="!pendingRoute"
              size="sm"
              variant="ghost"
              class="h-5 px-2 text-[12px] text-primary hover:text-primary hover:bg-primary/10"
            ...
            <Button
              ...
              size="sm"
              variant="ghost"
              class="h-5 px-2 text-[12px] text-destructive hover:text-destructive hover:bg-destructive/10 -mr-2"
        ...
        <div v-if="pendingRoute" class="flex items-center justify-between bg-muted/40 p-1.5 rounded border border-primary/20 mb-2">
          <span class="text-[12px] font-medium text-primary flex items-center gap-1">
        ...
        <div class="flex items-center gap-1.5 flex-wrap min-h-[22px]">
          <div v-if="(!selectedPlayer.route || selectedPlayer.route.segments.length === 0) && !pendingRoute" class="text-[12px] text-muted-foreground italic px-2">
            No route assigned
          </div>
          <div
            v-for="(seg, i) in (pendingRoute ? pendingRoute.segments : selectedPlayer.route?.segments)"
            :key="i"
            class="text-[12px] px-2 py-0.5 rounded-full border text-foreground font-medium flex items-center gap-1 transition-colors"
            :class="pendingRoute ? 'bg-primary/10 border-primary/30' : 'bg-muted border-border'"
          >
            <span>{{ seg.type }}</span>
            <span v-if="seg.readOrder" class="bg-primary text-primary-foreground text-[10px] w-3.5 h-3.5 flex items-center justify-center rounded-full -mr-1 shadow-sm">{{ seg.readOrder }}</span>
          </div>
        </div>
        ...
        <div v-if="selectedPlayer.route && selectedPlayer.route.segments.length > 0 && !pendingRoute" class="pt-1.5">
          <Button
            ...
            class="h-7 px-2 text-[12px] w-full"
      ...
      <div v-if="routeAnalytics" class="space-y-2 pt-2 border-t border-border/50">
        <div class="flex items-center justify-between">
          <p class="text-[12px] font-bold text-muted-foreground uppercase tracking-wider">
            {{ isRusher(selectedPlayer) ? 'Rush to QB' : 'Route analytics' }}
          </p>
          <span class="text-[11px] text-muted-foreground/60 font-mono">SPD: {{ playerSpeedAttr }}/10</span>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <div class="bg-muted/30 rounded px-2.5 py-1.5 border border-border/50">
            <p class="text-[11px] text-muted-foreground font-medium mb-0.5">
          ...
          <div class="bg-muted/30 rounded px-2.5 py-1.5 border border-border/50">
            ...
            <p class="text-sm font-mono font-bold leading-none text-foreground">{{ routeAnalytics.totalDistance.toFixed(1) }} <span class="text-[11px] font-normal text-muted-foreground">yd</span></p>
          </div>
        </div>

        <!-- Timeline: segment duration (rush path vs route segments) -->
        <div class="space-y-px pl-1 pt-1">
          <div v-for="(seg, i) in routeAnalytics.segments" :key="i" class="flex items-center text-[12px] gap-2 h-5 relative group">
            <!-- Connector line -->
            ...
            <span class="text-muted-foreground ml-auto font-mono text-[11px]">{{ seg.distance.toFixed(1) }}yd</span>
          </div>
        </div>
      </div>
    </div>

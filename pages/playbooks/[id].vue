<template>
  <div class="space-y-6">
    ...
    <div v-if="loading && plays.length === 0" class="text-muted-foreground text-sm">Loading plays...</div>
...
import type { Play, Playbook } from '~/lib/types'
...
async function handleSubmit(data: { name: string; playType: 'offense' | 'defense'; formation: string }) {
  if (editingPlay.value) {
    ...
    const starters = players.value.filter((p) =>
